...
const routing = {
	...
	'/code': path.join(__dirname, '/callback.html'),
	'/book': path.join(__dirname, '/data.json'),
...
const server = http.createServer((req, res) => {
	...
	if (filePath.includes('?code') && !routing[filePath]) {
		filePath = path.join(__dirname, '/callback.html');
	} else {
		filePath = routing[req.url] ? routing[req.url] : path.join(__dirname, req.url);
	}
